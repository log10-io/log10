...
@pytest.mark.chat
def test_prompt(openai_model):
    @prompt("Tell me a short joke", model=OpenaiChatModel(model=openai_model))
    def llm() -> str: ...
...
@pytest.mark.chat
@pytest.mark.stream
def test_prompt_stream(openai_model):
    @prompt("Tell me a short joke", model=OpenaiChatModel(model=openai_model))
    def llm() -> StreamedStr: ...
...
@pytest.mark.tools
def test_function_logging(openai_model):
    def activate_oven(temperature: int, mode: Literal["broil", "bake", "roast"]) -> str:
        """Turn the oven on with the provided settings."""
        return f"Preheating to {temperature} F with mode {mode}"

    @prompt(
        "Prepare the oven so I can make {food}", functions=[activate_oven], model=OpenaiChatModel(model=openai_model)
    )
...
async def test_async_parallel_stream_logging(openai_model):
    ...
    @prompt(
        ...
        model=OpenaiChatModel(model=openai_model),
    )
...
async def test_async_widget(openai_model):
    ...
    @prompt(
        ...
        Query: {query}
        """,  # noqa: E501
        model=OpenaiChatModel(openai_model, temperature=0.1, max_tokens=1000),
    )
