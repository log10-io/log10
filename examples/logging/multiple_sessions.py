...
with log10_session():
    third_prompt = PromptTemplate(
        input_variables=['month'], template='What is a good country to travel during {month}?',
    )

    chain_three = LLMChain(llm=llm, prompt=third_prompt)

    fourth_prompt = PromptTemplate(
        input_variables=['country_name'], template='Write a 1 day itinerary to {country_name}',
    )
