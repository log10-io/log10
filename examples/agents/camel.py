...
def camel_agent(userRole, assistantRole, taskPrompt, model, maxTurns, hparams):
...
Never say <CAMEL_TASK_DONE> unless my responses have solved your task."""
        ...
        for i in range(maxTurns):
            ...
            if 'claude' in model:
                ...
                assistant_completion = anthropicClient.completion(
                    prompt=messages,
            ...
            if 'claude' in model:
                ...
                user_completion = anthropicClient.completion(
                    prompt=messages,
                    stop_sequences=[anthropic.HUMAN_PROMPT],
                    model=model,
                    **hparams
                ...
                user_completion = openai.ChatCompletion.create(
...
Even if I told you that the task is completed in the context above you should still reply with a complete solution. Never tell me the task is completed or ask for the next request, but instead replay the final solution back to me."""
                ...
                if 'claude' in model:
                    summary_completion = anthropicClient.completion(
...
def run_camel_agent(userRole, assistantRole, taskPrompt, model, maxTurns, hparams={}):
    if isinstance(userRole, (str)) and isinstance(assistantRole, (str)) and isinstance(taskPrompt, (str)) and isinstance(model, (str)) and isinstance(maxTurns, (int, float)):
        with log10_session():
            user_messages, assistant_messages = camel_agent(
                userRole, assistantRole, taskPrompt, model, maxTurns, hparams)
        return user_messages, assistant_messages
    else:
...
run_camel_agent(userRole='Stock Trader', assistantRole='Python Programmer',
                taskPrompt='Develop a trading bot for the stock market', model=model, maxTurns=maxTurns, hparams=hparams)
