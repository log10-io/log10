...
run_camel_agent(userRole='C developer', assistantRole='Cybersecurity expert',
                taskPrompt="#include <stdio.h>\n#include <string.h>\n\nint main() {\n    char password[8];\n    int granted = 0;\n\n    printf(\"Enter password: \");\n    scanf(\"%s\", password);\n\n    if (strcmp(password, \"password\") == 0) {\n        granted = 1;\n    }\n\n    if (granted) {\n        printf(\"Access granted.\\n\");\n    } else {\n        printf(\"Access denied.\\n\");\n    }\n\n    return 0;\n}", model=model, maxTurns=maxTurns,
...
# run_camel_agent(userRole='Sales email copyeditor', assistantRole='Sales email copywriter',
#                 taskPrompt='Write a sales email to Pfizer about a new healthcare CRM', model=model, maxTurns=maxTurns,
#                 module=module, hparams=hparams)

# run_camel_agent(userRole='Poor PhD Student', assistantRole='Experienced Computational Chemist',
#                 taskPrompt='Perform a molecular dynamics solution of a molecule: CN1CCC[C@H]1c2cccnc2. Design and conduct a 100 ns molecular dynamics simulation of the molecule CN1CCC[C@H]1c2cccnc2 in an explicit solvent environment using the CHARMM force field and analyze the conformational changes and hydrogen bonding patterns over time', model=model, maxTurns=maxTurns,
#                 module=module, hparams=hparams)

# run_camel_agent(userRole='Web3 guru', assistantRole='Hindi translator',
#                 taskPrompt='Write a blog post about web3 in Hindi', model=model, maxTurns=maxTurns,
#                 module=module, hparams=hparams)
