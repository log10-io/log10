...
from ._log10_utils import create_log10_test_run
...
class JSONReport(JSONReportBase):
    ...
    def pytest_sessionstart(self, session):
        ...
        if self.log10_test_run:
            self._eval_session_name = self.log10_test_run.get('name')
            test_id = self.log10_test_run.get('id')
        else:
            root = Path(session.fspath)
            self._eval_session_name = (
                self._config.getoption('eval_session_name') or
                self._config.getini('eval_session_name') or
                root.name
            )
    ...
    def pytest_sessionfinish(self, session):
        ...
        if not path:
            path = f"{self._session_id}.report.json"
        ...
        if path:
            ...
            if not self._config.option.local:
    ...
    def pytest_terminal_summary(self, terminalreporter):
        ...
        if self.log10_test_run:
            terminalreporter.write_sep('-', 'Log10 test run')
            terminalreporter.write_line(f"Log10 test run is enabled. Test run: {self.log10_test_run.get('name')}-{self.log10_test_run.get('id')}")
...
def pytest_addoption(parser):
    ...
    group.addoption(
        '--json-report', default=True, action='store_true',
        help='create JSON report (enabled by default)')
    group.addoption(
        '--json-report-file', default=None,
        help='target path to save JSON report report')
    group.addoption(
    ...
    group.addoption(
        ...
        action='store',
        default=None,
        help='Name for the evaluation session in JSON report'
    )
    parser.addini(
        'eval_session_name',
        'Name for the evaluation session in JSON report',
        default=None
    )
...
def pytest_configure(config):
    ...
    eval_name = (
        config.getoption('eval_session_name') or
        config.getini('eval_session_name') or
        Path(config.rootdir).name
    )
