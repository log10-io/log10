...
def initialize_bigquery():
    ...
    if dataset_exists(dataset_id):
        _logger.debug(f"Dataset {dataset_id} exists.")
    else:
        _logger.debug(f"Dataset {dataset_id} does not exist. Creating...")
        # Create the dataset
        ...
        _logger.debug(f"Dataset {created_dataset.dataset_id} created.")

    client_dataset = client.dataset(dataset_id)

    # Check if table exists
    if table_exists(dataset_id, completions_table_id):
        _logger.debug(f"Table {completions_table_id} exists in dataset {dataset_id}.")
    else:
        _logger.debug(
        ...
        _logger.debug(f"Table {created_table.table_id} created.")
