...
def _init_log_row(func, *args, **kwargs):
    ...
    if "messages" in kwargs_copy:
        kwargs_copy["messages"] = filter_large_images(
            flatten_messages(kwargs_copy["messages"])
        )
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            with timed_block("result call duration (sync)"):
                ...
                if "anthropic" in func.__module__:
                    if (
                        type(output).__name__ == "Stream"
                        or "MessageStreamManager" in type(output).__name__
                    ):
